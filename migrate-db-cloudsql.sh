...
function run_sql {
    psql postgresql://postgres:${POSTGRES_DEST_PASSWORD}@${DEST_INSTANCE_IP}/postgres << EOF
        ...
        ALTER DEFAULT PRIVILEGES FOR ROLE "${3}" IN SCHEMA public GRANT SELECT ON SEQUENCES to developer;
        ALTER DEFAULT PRIVILEGES FOR ROLE "${3}" IN SCHEMA public GRANT SELECT ON TABLES to developer;
EOF
}
